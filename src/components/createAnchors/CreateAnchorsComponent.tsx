...
export const CreateAnchorComponent = () => {
  ...
  const [selectedLocationDictionary, setSelectedLocationDictionary] = useState<{ [key: string]: any }>({});     // list of the selected location
  const [newLocationDictionary, setNewLocationDictionary] = useState<{ [key: string]: any }>({room_id: "", lat: null, lon: null, floor_nr: null, building_id:"", address_string: "", campus_id: ""});  // new location_id as string
  ...
  const [selectedBuildingDictionary, setSelectedBuildingDictionary] = useState<{ [key: string]: any }>({});     // list of the selected building
  const [newBuildingDictionary, setNewBuildingDictionary] = useState<{ [key: string]: any }>({building_id:"", address_string: "", campus_id: ""});  // new user building as dictionary
  ...
  useEffect(() => {
      ...
      if (getTagsFromDB().includes(newTagString) || temporaryTagList.includes(newTagString) || newTagString == "") {
        setNewTagError(true);
      }
      else {
  ...
  const createNewTempTag = () => {
    if (newTagString != "") {
      if (getTagsFromDB().includes(newTagString) || temporaryTagList.includes(newTagString)) {
      }
      else {
  ...
  const closeDialogCreateTag = () => {
    var dialogCreateTag = document.getElementById("dialogCreateTag") as HTMLIonModalElement;
  ...
  const closeDialogSelectTag = () => {
    var dialogSelectTag = document.getElementById("dialogSelectTag") as HTMLIonModalElement;
  ...
  const getLocationsFromFilter = () => {
    ...
    while (i < allLocations.length) {
      ...
      if (value.room_id.startsWith(filterLocationString)){
        ...
        if (value.room_id != "" || value.campus_id != "" || value.building_id != "" || value.floor_nr != null) {
          var selectedLocationRoom = document.createElement("ion-label");
          var innerHTMLString = ""
          if (value.room_id != "") {innerHTMLString += '<span style="font-weight: bold">' + value.room_id + '</span> '}
          if (value.campus_id != "" || value.building_id != "" || value.floor_nr != null) {
            innerHTMLString += '(';
            if (value.campus_id != "") {innerHTMLString += value.campus_id + ', '};
            if (value.building_id != "") {innerHTMLString += value.building_id + ', '};
            if (value.floor_nr != null) {innerHTMLString += value.floor_nr + ', '};
  ...
  useEffect(() => {
      ...
      if (newLocationDictionary.room_id == "" && newLocationDictionary.lat == null && newLocationDictionary.lon == null && newLocationDictionary.floor_nr == null && newLocationDictionary.building_id == "" && newLocationDictionary.campus_id == "" && newLocationDictionary.address_string =="") {
  ...
  const readNewLocationInputFloor = (event: CustomEvent) => {
    const newLocationDictValue : { [key: string]: any; } = Object.assign({}, newLocationDictionary);
    if (event.detail.value == "") {newLocationDictValue.floor_nr = null}
    else {newLocationDictValue.floor_nr = Math.round(event.detail.value)};
    setNewLocationDictionary(newLocationDictValue);
  };
  ...
  const resetConstantsTempLocation = () => {
    setNewLocationDictionary({room_id: "", lat: null, lon: null, floor_nr: null, building_id:"", address_string: "", campus_id: ""});
    setSelectedLocationDictionary({});
  };

  // closing the dialog (modal) to enter a new location
  const closeDialogCreateLocation = () => {
    var dialogCreateLocation = document.getElementById("dialogCreateLocation") as HTMLIonModalElement;
  ...
  const closeDialogSelectLocation = () => {
    var dialogSelectLocation = document.getElementById("dialogSelectLocation") as HTMLIonModalElement;
  ...
  const getBuildingsFromFilter = () => {
    ...
    const allbuildingsDB = locationsDB.map((location : {building_id: string, address_string: string, campus_id: string})  => ({
      building_id: location.building_id,
      address_string: location.address_string,
      campus_id: location.campus_id
    }));
    ...
    allBuildings.sort((a : any, b : any) => {
      ...
      else if (buildingA > buildingB) {return 1;}
      else {return 0};                      // same room
    });
    // temporary storage of the current selection for subsequent editing
    ...
    while (i < allBuildings.length) {
      ...
      if (value.building_id.startsWith(filterBuildingString)){
        ...
        if (value.building_id != "" || value.campus_id != "") {
          ...
          var innerHTMLString = ""
          if (value.building_id != "") {innerHTMLString += '<span style="font-weight: bold">' + value.building_id + '</span> '}
          if (value.campus_id != "") {
            innerHTMLString += '(';
            if (value.campus_id != "") {innerHTMLString += value.campus_id + ', '};
            innerHTMLString = innerHTMLString.slice(0, -2) + ")";
  ...
  const readNewBuildingInputBuilding = (event: CustomEvent) => {
    const newBuildingDictValue: { [key: string]: any; } = Object.assign({}, newBuildingDictionary);
  ...
  const resetConstantsTempBuilding = () => {
    setNewBuildingDictionary({building_id:"", address_string: "", campus_id: ""});
    setSelectedBuildingDictionary({});
  };
  
  // closing the dialog (modal) to enter a new location
  const closeDialogCreateBuilding = () => {
    var dialogCreateBuilding = document.getElementById("dialogCreateBuilding") as HTMLIonModalElement;
  ...
  const closeDialogSelectBuilding = () => {
    var dialogSelectBuilding = document.getElementById("dialogSelectBuilding") as HTMLIonModalElement;
    ...
    if (Object.keys(selectedBuildingDictionary).length == 0) {
      ...
      if (selectedBuildingDictionary.building_id != "") {
        if (selectedBuildingDictionary.address_string != "" || 
            ...
            if (selectedBuildingDictionary.address_string != "" ) {innerHTMLString += selectedBuildingDictionary.address_string}
            else if (selectedBuildingDictionary.campus_id != "") {innerHTMLString += selectedBuildingDictionary.campus_id}
        }
        else {
          innerHTMLString += selectedBuildingDictionary.building_id;
        };
  ...
  useEffect(() => {
    setLocalAnchor({
      ...
      campus_id: selectedLocationDictionary.campus_id !== "" ? selectedLocationDictionary.campus_id : undefined,
    });
  ...
  useEffect(() => {
      ...
      if (getGroupsFromDB().includes(newGroupString) || temporaryGroupList.includes(newGroupString) || newGroupString == "") {
  ...
  const createNewTempGroup = () => {
    if (newGroupString != "") {
      if (getGroupsFromDB().includes(newGroupString) || temporaryGroupList.includes(newGroupString)) {
      }
      else {
  ...
  const closeDialogCreateGroup = () => {
    var dialogCreateGroup = document.getElementById("dialogCreateGroup") as HTMLIonModalElement;
  ...
  const closeDialogSelectGroup = () => {
    var dialogSelectGroup = document.getElementById("dialogSelectGroup") as HTMLIonModalElement;
  ...
  return (
    <IonPage>
      ...
      <IonContent className="ion-padding" fullscreen>
        ...
        <IonButton id="openDialogSelectTag" expand="block" color="light" fill="solid" size="default">
          <div>
            <IonLabel id="openDialogSelectTagLabel" class="ion-text-wrap">Tags</IonLabel>
        ...
        <IonModal id="dialogSelectTag" trigger="openDialogSelectTag" onDidPresent={getTagsFromFilter} onDidDismiss={updateTagInput}>
            <IonHeader>
              ...
              <IonSearchbar onIonInput={updateTagFilter} color="light" id="tagSearchBar"></IonSearchbar>
            </IonHeader>
            <IonContent>
              <IonInfiniteScroll>
                <IonList id="listFilteredTags">
                </IonList>
              </IonInfiniteScroll>
            </IonContent>
            <IonFooter>
              <IonGrid>
                <IonRow>
                  <IonCol>
                    <IonButton id="openDialogCreateTag" expand="full" color="primary">Neues Tag</IonButton>
                  </IonCol>
                  <IonCol>
                    <IonButton onClick={closeDialogSelectTag} id="cancelSelectTag" expand="full" color="primary">Speichern</IonButton>
                  </IonCol>
                </IonRow>
              </IonGrid>
              ...
              <IonModal id="dialogCreateTag" trigger="openDialogCreateTag">
                ...
                <IonContent class="padding">
                  <IonGrid>
                    <IonRow>
                      <IonCol>
                      <IonInput onIonInput={readNewTagInput} color="dark" labelPlacement="stacked" type="text" fill="outline" style={{ margin: "16px 0 16px 0" }} placeholder="Tag">
                        <div slot="label">Tag <IonText color="danger"> (Pflichtfeld) </IonText></div>
                      </IonInput>
                      </IonCol>
                    </IonRow>
                  </IonGrid>
                ...
                <IonGrid>
                  <IonRow>
                    <IonCol>
                      <IonButton onClick={closeDialogCreateTag} id="cancelTempTag" expand="full" color="primary">Abbrechen</IonButton>
                    </IonCol>
                    <IonCol>
                      <IonButton onClick={createNewTempTag} id="saveTempTag" expand="full" color="primary">Speichern</IonButton>
                    </IonCol>
                  </IonRow>
                </IonGrid>
                </IonFooter>
        ...
        <IonToggle onIonChange={addDate} labelPlacement="start">Termin</IonToggle>
        </IonItem>
          ...
          <IonModal keepContentsMounted={true} id="dialogSelectDateStart">
            <IonHeader>
              <IonToolbar>
                ...
                <IonButtons slot="end">
                  <IonButton onClick={() => (document.getElementById("dialogSelectDateStart") as HTMLIonModalElement).dismiss()}>
                    <IonIcon icon={closeOutline} size="large"></IonIcon>
                  </IonButton>
                </IonButtons>
              </IonToolbar>
          ...
          <IonModal keepContentsMounted={true} id="dialogSelectDateEnd">
            <IonHeader>
              <IonToolbar>
                ...
                <IonButtons slot="end">
                  <IonButton onClick={() => (document.getElementById("dialogSelectDateEnd") as HTMLIonModalElement).dismiss()}>
                    <IonIcon icon={closeOutline} size="large"></IonIcon>
                  </IonButton>
                </IonButtons>
              </IonToolbar>
        ...
        <IonToggle onIonChange={addValid} labelPlacement="start">Gültigkeit</IonToggle>
        </IonItem>
          ...
          <IonModal keepContentsMounted={true} id="dialogSelectValidStart">
            <IonHeader>
              <IonToolbar>
                <IonTitle slot="start">Startzeit auswählen</IonTitle>
                <IonButtons slot="end">
                  <IonButton onClick={() => (document.getElementById("dialogSelectValidStart") as HTMLIonModalElement).dismiss()}>
                    <IonIcon icon={closeOutline} size="large"></IonIcon>
                  </IonButton>
                </IonButtons>
              </IonToolbar>
          ...
          <IonModal keepContentsMounted={true} id="dialogSelectValidEnd">
            <IonHeader>
              <IonToolbar>
                <IonTitle slot="start">Endzeit auswählen</IonTitle>
                <IonButtons slot="end">
                  <IonButton onClick={() => (document.getElementById("dialogSelectValidEnd") as HTMLIonModalElement).dismiss()}>
                    <IonIcon icon={closeOutline} size="large"></IonIcon>
                  </IonButton>
                </IonButtons>
              </IonToolbar>
        ...
        <IonButton id="openDialogSelectLocation" expand="block" color="light" fill="solid" size="default">
          <div>
            <IonLabel id="openDialogSelectLocationLabel" class="ion-text-wrap">Ort</IonLabel>
            <IonIcon icon={addCircleOutline} size="large" aria-hidden="true"></IonIcon>
          </div>
        </IonButton>
        ...
        <IonModal id="dialogSelectLocation" trigger="openDialogSelectLocation" onDidPresent={getLocationsFromFilter} onDidDismiss={updateLocationsInput}>
            <IonHeader>
              <IonToolbar>
              ...
              <IonSearchbar onIonInput={updateLocationFilter} color="light" id="locationSearchBar"></IonSearchbar>
            </IonHeader>
            <IonContent>
              <IonInfiniteScroll>
                <IonList id="listFilteredLocations">
                </IonList>
              </IonInfiniteScroll>
            </IonContent>
            <IonFooter>
              <IonGrid>
                <IonRow>
                  <IonCol>
                    <IonButton onClick={resetConstantsTempLocation} id="openDialogCreateLocation" expand="full" color="primary">Neuer Ort</IonButton>
                  </IonCol>
                  <IonCol>
                    <IonButton onClick={closeDialogSelectLocation} id="cancelSelectLocation" expand="full" color="primary">Speichern</IonButton>
                  </IonCol>
                </IonRow>
              </IonGrid>
              ...
              <IonModal id="dialogCreateLocation" trigger="openDialogCreateLocation">
                ...
                <IonContent class="padding">
                  <IonGrid>
                    <IonRow>
                      <IonCol>
                      <IonInput onIonInput={readNewLocationInputRoom} color="dark" label="Raum" labelPlacement="stacked" type="text" fill="outline" style={{ margin: "16px 0 16px 0" }} placeholder="Raum"></IonInput>
                      <IonButton id="openDialogSelectPosition" expand="block" color="light" fill="solid" size="default">
                        <div>
                          <IonLabel id="openDialogSelectPositionLabel" class="ion-text-wrap">Position auf Karte</IonLabel>
                          <IonIcon icon={addCircleOutline} size="large" aria-hidden="true"></IonIcon>
                        </div>
                      </IonButton>
                      ...
                      <IonInput onIonInput={readNewLocationInputFloor} color="dark" label="Stockwerk" labelPlacement="stacked" type="number" fill="outline" style={{ margin: "16px 0 16px 0" }} placeholder="Stockwerk"></IonInput>
                      <IonButton id="openDialogSelectBuilding" expand="block" color="light" fill="solid" size="default">
                        <div>
                          <IonLabel id="openDialogSelectBuildingLabel" class="ion-text-wrap">Gebäude</IonLabel>
                          <IonIcon icon={addCircleOutline} size="large" aria-hidden="true"></IonIcon>
                        </div>
                      </IonButton>
                      ...
                      <IonModal id="dialogSelectBuilding" trigger="openDialogSelectBuilding" onDidPresent={getBuildingsFromFilter} onDidDismiss={updateBuildingsInput}>
                        <IonHeader>
                          <IonToolbar>
                          ...
                          <IonSearchbar onIonInput={updateBuildingFilter} color="light" id="buildingSearchBar"></IonSearchbar>
                        </IonHeader>
                        <IonContent>
                          <IonInfiniteScroll>
                            <IonList id="listFilteredBuildings">
                            </IonList>
                          </IonInfiniteScroll>
                        </IonContent>
                        <IonFooter>
                          <IonGrid>
                            <IonRow>
                              <IonCol>
                                <IonButton onClick={resetConstantsTempBuilding} id="openDialogCreateBuilding" expand="full" color="primary">Neues Gebäude</IonButton>
                              </IonCol>
                              <IonCol>
                                <IonButton onClick={closeDialogSelectBuilding} id="cancelSelectBuilding" expand="full" color="primary">Speichern</IonButton>
                              </IonCol>
                            </IonRow>
                          </IonGrid>
                          {/* overlay (modal) for the creation of buildings */}
                          <IonModal id="dialogCreateBuilding" trigger="openDialogCreateBuilding">
                            <IonHeader>
                              <IonToolbar>
                            ...
                            <IonContent class="padding">
                              <IonGrid>
                                <IonRow>
                                  <IonCol>
                                    <IonInput onIonInput={readNewBuildingInputBuilding} color="dark" label="Gebäudename" labelPlacement="stacked" type="text" fill="outline" style={{ margin: "16px 0 16px 0" }} placeholder="Gebäudenamen"></IonInput>
                                    <IonInput onIonInput={readNewBuildingInputAddress} color="dark" label="Adresse" labelPlacement="stacked" type="text" fill="outline" style={{ margin: "16px 0 16px 0" }} placeholder="Adresse"></IonInput>
                                    <IonInput onIonInput={readNewBuildingInputCampus} color="dark" label="Campus" labelPlacement="stacked" type="text" fill="outline" style={{ margin: "16px 0 16px 0" }} placeholder="Campus"></IonInput>
                                  </IonCol>
                                </IonRow>
                              </IonGrid>
                            </IonContent>
                            <IonFooter>
                              <IonGrid>
                                <IonRow>
                                  <IonCol>
                                    <IonButton onClick={closeDialogCreateBuilding} id="cancelTempBuilding" expand="full" color="primary">Abbrechen</IonButton>
                                  </IonCol>
                                  <IonCol>
                                    <IonButton onClick={createNewTempBuilding} id="saveTempBuilding" expand="full" color="primary">Speichern</IonButton>
                                  </IonCol>
                                </IonRow>
                              </IonGrid>
                            </IonFooter>
                ...
                <IonGrid>
                  <IonRow>
                    <IonCol>
                      <IonButton onClick={closeDialogCreateLocation} id="cancelTempLocation" expand="full" color="primary">Abbrechen</IonButton>
                    </IonCol>
                    <IonCol>
                      <IonButton onClick={createNewTempLocation} id="saveTempLocation" expand="full" color="primary">Speichern</IonButton>
                    </IonCol>
                  </IonRow>
                </IonGrid>
                </IonFooter>
       ...
       <IonButton id="openDialogSelectGroup" expand="block" color="light" fill="solid" size="default">
          <div>
            <IonLabel id="openDialogSelectGroupLabel" class="ion-text-wrap">Gruppe</IonLabel>
            <IonIcon icon={addCircleOutline} size="large" aria-hidden="true"></IonIcon>
          </div>
        </IonButton>
        ...
        <IonModal id="dialogSelectGroup" trigger="openDialogSelectGroup" onDidPresent={getGroupsFromFilter} onDidDismiss={updateGroupsInput}>
            <IonHeader>
              <IonToolbar>
              ...
              <IonSearchbar onIonInput={updateGroupFilter} color="light" id="groupSearchBar"></IonSearchbar>
            </IonHeader>
            <IonContent>
              <IonInfiniteScroll>
                <IonList id="listFilteredGroups">
                </IonList>
              </IonInfiniteScroll>
            </IonContent>
            <IonFooter>
              <IonGrid>
                <IonRow>
                  <IonCol>
                    <IonButton id="openDialogCreateGroup" expand="full" color="primary">Neue Gruppe</IonButton>
                  </IonCol>
                  <IonCol>
                    <IonButton onClick={closeDialogSelectGroup} id="cancelSelectGroup" expand="full" color="primary">Speichern</IonButton>
                  </IonCol>
                </IonRow>
              </IonGrid>
              ...
              <IonModal id="dialogCreateGroup" trigger="openDialogCreateGroup">
                ...
                <IonContent class="padding">
                  <IonGrid>
                    <IonRow>
                      <IonCol>
                      <IonInput onIonInput={readNewGroupInput} color="dark" labelPlacement="stacked" type="text" fill="outline" style={{ margin: "16px 0 16px 0" }} placeholder="Gruppe">
                        <div slot="label">Gruppe <IonText color="danger"> (Pflichtfeld) </IonText></div>
                      </IonInput>
                      </IonCol>
                    </IonRow>
                  </IonGrid>
                ...
                <IonGrid>
                  <IonRow>
                    <IonCol>
                      <IonButton onClick={closeDialogCreateGroup} id="cancelTempGroup" expand="full" color="primary">Abbrechen</IonButton>
                    </IonCol>
                    <IonCol>
                      <IonButton onClick={createNewTempGroup} id="saveTempGroup" expand="full" color="primary">Speichern</IonButton>
                    </IonCol>
                  </IonRow>
                </IonGrid>
                </IonFooter>
        ...
        <IonButton
          ...
          onClick={() => {
            ...
            <div>
              <IonLabel id="openDialogSelectDocumentLabel" class="ion-text-wrap">Dokumente</IonLabel>
              <IonIcon icon={addCircleOutline} size="large" aria-hidden="true"></IonIcon>
            </div>
        </IonButton>
      ...
      <IonFooter style={{ display: "flex", justifyContent: "center", backgroundColor: "#fff"}}>
        <IonButton
          id="create-anchor"
          fill="solid"
          strong={true}
        ...
        <IonToast
          ...
          position="top"
          message={newTagError ? "Erstellung fehlgeschlagen! \nEingabe ungültig oder Tag bereits vorhanden." : "Tag erfolgreich erstellt"}
          duration={ 1200 }
        ...
        <IonToast
          ...
          position="top"
          message={newLocationError ? "Erstellung fehlgeschlagen! \nKeine Werte eingegeben." : "Ort erfolgreich erstellt"}
          duration={ 1200 }
        ...
        <IonToast
          ...
          position="top"
          message={newBuildingError ? "Erstellung fehlgeschlagen! \nKeine Werte eingegeben." : "Gebäude erfolgreich erstellt"}
          duration={ 1200 }
        ...
        <IonToast
          ...
          position="top"
          message={newGroupError ? "Erstellung fehlgeschlagen! \nEingabe ungültig oder Gruppe bereits vorhanden." : "Gruppe erfolgreich erstellt"}
          duration={ 1200 }
