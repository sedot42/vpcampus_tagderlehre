...
import { closeOutline, homeOutline, layersOutline, mapOutline, alertCircleOutline } from "ionicons/icons";
...
export const MapAnchorComponent = () => {
  ...
  const updateMapBackground = () => {
    if (mapContainerRef.current) {
      ...
      mapContainerRef.current.eachLayer((layer) => {if (layer.options.pane === "tilePane") {mapContainerRef.current!.removeLayer(layer)}});
  ...
  function backgroundLayerFloorplan(floor: number) {
    const bgLayerFloorPlan = tileLayer.wms("http://localhost:8080/geoserver/wms?", {
      ...
      minZoom:18,
      maxZoom:22,
      attribution:"<a href = 'https://www.baudokumentation.ch/projekt/fhnw-campus-muttenz/734718'>Schweizer Baudokumentation</a>"
    });
  ...
  function backgroundLayerAmtlicheVermessung() {
    const bgLayerAmtlicheVermessung = tileLayer.wms("https://wfs.geodienste.ch/av_situationsplan_0/deu?", {
  ...
  const filterAnchorFromServer = (serverAnchorData: any) => {
    ...
    for (const anchor of serverAnchorData) {
      ...
      const tagFilter: any = JSON.parse(localStorage.getItem('campus_v_p_selTags') || "[]");
      const groupFilter: any = JSON.parse(localStorage.getItem('campus_v_p_selGroups') || "[]");
      if (anchor.tags.some((element: string) => tagFilter.includes(element)) || groupFilter.includes(anchor.group_id) || (tagFilter.length == 0 && groupFilter.length == 0)) {
    ...
    const idsInFilteredAnchorListHasDate = new Set(filteredAnchorListHasDate.map((d:any) => d.id));
    filteredAnchorListIsValid = filteredAnchorListIsValid.filter((d:any) => !idsInFilteredAnchorListHasDate.has(d.id));
    const idsInFilteredAnchorListHasDateWrongFloor = new Set(filteredAnchorListHasDateWrongFloor.map((d:any) => d.id));
    filteredAnchorListIsValidWrongFloor = filteredAnchorListIsValidWrongFloor.filter((d:any) => !idsInFilteredAnchorListHasDateWrongFloor.has(d.id));
    // retrun lists
    return [filteredAnchorListHasDate, 
  ...
  const createMarkerStyle = (color:string = '#44a2fa', rotation:string = '45deg', uidAnchor:string = "", size:number = 3) => {
  ...
  const calculateMarkerOrientation = (anchor:any, mapCenter:any, mapBoundLeft:number, mapBoundRight:number, mapBoundTop:number, mapBoundBottom:number) => {
    ...
    const positionLat = () => {
      if ((mapCenter.lat - anchor.lat) >= (mapCenter.lat - mapBoundBottom)) {return mapBoundBottom}
  ...
  const addMarkerToMap = (anchor:any, mapCenter:any, mapBounds: any, color:string, size:number) => {
    ...
    if (anchor.lat != null && anchor.lon != null) {
      ...
      const mapBoundLeft = mapBounds.getSouthWest().lng + ((Math.abs(mapBounds.getSouthWest().lng - mapBounds.getNorthEast().lng) / document.getElementById('mapContainer')!.clientWidth) * (size / 2 * 16) * Math.sqrt(2));
      const mapBoundRight = mapBounds.getNorthEast().lng - ((Math.abs(mapBounds.getSouthWest().lng - mapBounds.getNorthEast().lng) / document.getElementById('mapContainer')!.clientWidth) * (size / 2 * 16) * Math.sqrt(2));
      const mapBoundTop = mapBounds.getNorthEast().lat - ((Math.abs(mapBounds.getSouthWest().lat - mapBounds.getNorthEast().lat) / document.getElementById('mapContainer')!.clientHeight) * (size / 2 * 16) * ((3 / size) + Math.sqrt(2)));
      const mapBoundBottom = mapBounds.getSouthWest().lat + ((Math.abs(mapBounds.getSouthWest().lat - mapBounds.getNorthEast().lat) / document.getElementById('mapContainer')!.clientHeight) * (size / 2 * 16) * (-(3 / size) + Math.sqrt(2)));
      //if anchor in display borders
      if (anchor.lat >= mapBoundBottom && anchor.lat <= mapBoundTop && anchor.lon >= mapBoundLeft && anchor.lon <= mapBoundRight) {
        const mapPositionMarker = new Marker([anchor.lat, anchor.lon], {icon: createMarkerStyle(color, '45deg', anchor.id, size)});
      ...
      // calculate position and rotation of anchor icon (anchor not in display borders)
        ...
        var markerOrientation = calculateMarkerOrientation(anchor, mapCenter, mapBoundLeft, mapBoundRight, mapBoundTop, mapBoundBottom);
        var mapPositionMarker = new Marker([markerOrientation[0], markerOrientation[1]], {icon: createMarkerStyle(color, markerOrientation[2]+'deg', anchor.id, size)});
        mapPositionMarker.addEventListener('click', (e)=>{mapContainerRef.current!.panTo([anchor.lat, anchor.lon])})
  ...
  const updateAnchorOnMap = () => {
    ...
    mapContainerRef.current!.eachLayer((layer) => {if (layer.options.pane === 'markerPane') {mapContainerRef.current!.removeLayer(layer)}});
  ...
  function openAnchorInformation (clickEvent:any, anchorID:any) {    
    ...
    for (const anchor of filteredAnchorData[0].concat(filteredAnchorData[2])) {
      ...
      if (anchor.lat == markerposition.lat && anchor.lon == markerposition.lng) {
        ...
        anchorInfoDescrib.innerHTML = anchor.anchor_description != ""? anchor.anchor_description : "";
        ...
        anchorInfoPlaceValue.innerHTML = anchor.room_id != "" ? anchor.room_id : anchor.lat.toFixed(5) + " / " + anchor.lon.toFixed(5);
        ...
        anchorInfoStartValue.innerHTML = startDate == ""? "" : startDate.getDate() + '.' + Number(startDate.getMonth()+1) + '.' + startDate.getFullYear() + ', ' + startDate.getHours() + ':' + startDate.getMinutes().toString().padStart(2, '0') + ' Uhr';  
        ...
        anchorInfoEndValue.innerHTML = endDate == ""? "" : endDate.getDate() + '.' + Number(endDate.getMonth()+1) + '.' + endDate.getFullYear() + ', ' + endDate.getHours() + ':' + endDate.getMinutes().toString().padStart(2, '0') + ' Uhr';  
        ...
        const startVDate = anchor.valid_from != undefined? new Date(anchor.valid_from) : ""
        anchorInfoStartVValue.innerHTML = startVDate == ""? "" : startVDate.getDate() + '.' + Number(startVDate.getMonth()+1) + '.' + startVDate.getFullYear() + ', ' + startVDate.getHours() + ':' + startVDate.getMinutes().toString().padStart(2, '0') + ' Uhr';  
        ...
        const endVDate = anchor.valid_until != undefined? new Date(anchor.valid_until) : ""
        anchorInfoEndVValue.innerHTML = endVDate == ""? "" : endVDate.getDate() + '.' + Number(endVDate.getMonth()+1) + '.' + endVDate.getFullYear() + ', ' + endVDate.getHours() + ':' + endVDate.getMinutes().toString().padStart(2, '0') + ' Uhr';  
  ...
  function GetInformationDisplayedMap() {
    const map = useMapEvents({
      move: (e) => {updateAnchorOnMap();},
      resize: (e) => {setMapIsLoad(true)}
    });
    return null;
  };
  ...
  const switchLayerSelection = () => {
    if (layerSettingsVisible) {
      ...
      (document.getElementById('layerMenuButton')! as HTMLIonButtonElement).setAttribute("color", "primary")
      ...
      (document.getElementById('layerMenuButton')! as HTMLIonButtonElement).setAttribute("color", "tertiary")
      document.getElementById('layerMenuButtonST')!.style.zIndex = "400";
      document.getElementById('layerMenuButtonFP')!.style.zIndex = "400";
  ...
  const selectBackgroundST = () => {
    if (layerFloorplanVisible === true) {
      ...
      (document.getElementById('layerMenuButtonST')! as HTMLIonButtonElement).setAttribute("color", "tertiary");
      (document.getElementById('layerMenuButtonFP')! as HTMLIonButtonElement).setAttribute("color", "primary");
      document.getElementById('layerMenuFloorSlider')!.style.zIndex = "-400";
    };
  };
  ...
  const selectBackgroundFP = () => {
    if (layerFloorplanVisible === false) {
      ...
      (document.getElementById('layerMenuButtonST')! as HTMLIonButtonElement).setAttribute("color", "primary");
      (document.getElementById('layerMenuButtonFP')! as HTMLIonButtonElement).setAttribute("color", "tertiary");
      document.getElementById('layerMenuFloorSlider')!.style.zIndex = "400";
    };
  };
  ...
  const updateAnchorInTimeline = () => { 
    ...
    function prepareDictForLayering(dictAnchors: any, displayTyp: string ) {
      ...
      for (const indivAnchor of dictAnchors) {
        ...
        dictListForLayeringTemp.push({id: indivAnchor.id,
                                  startTime: selectedDayFilter.getDate() === startTimeAnchor.getDate() && selectedDayFilter.getMonth() === startTimeAnchor.getMonth() ? 
                                             startTimeAnchor.getHours()+startTimeAnchor.getMinutes()/60 : 0,
                                  endTime: selectedDayFilter.getDate() === endTimeAnchor.getDate() && selectedDayFilter.getMonth() === endTimeAnchor.getMonth() ? 
                                           endTimeAnchor.getHours()+endTimeAnchor.getMinutes()/60 : 24,
                                  display: displayTyp,
                                  lat: indivAnchor.lat,
                                  lon: indivAnchor.lon,
    ...
    const anchorInTimeSelectionLayering = prepareDictForLayering(anchorInTimeSelection, 'inTime');
    const anchorOutTimeSelectionLayering = prepareDictForLayering(anchorOutTimeSelection, 'outTime');
    const dictListForLayering = anchorInTimeSelectionLayering.concat(anchorOutTimeSelectionLayering);
    ...
    selectedTimeSpace.style.width = 335 / 24 * (Number(endTimeFilter) - Number(startTimeFilter)) + 'px';
    ...
    while (i < endTimeFilter-startTimeFilter) {
      ...
      verticalLine.style.left = 335 / 24 * Number(startTimeFilter) + 335 / 24 * i - 1 + 'px';
    ...
    if (numberOfLayer > 0) {
      ...
      while (i < numberOfLayer){
        for (const anchorObj of displayLayering[i]) {
          ...
          anchorObjButton.style.background = anchorObj.display == 'inTime' ? '#44a2fa': '#ffffff';
          anchorObjButton.style.borderRadius = '2px'
          anchorObjButton.style.border = anchorObj.display == 'inTime' ? '2px solid #ffffff': '2px dashed #44a2fa';
          anchorObjButton.style.height = (60 - ((numberOfLayer - 1) * 6)) / numberOfLayer + 'px';
          anchorObjButton.style.width = 335 / 24 * (Number(anchorObj.endTime) - Number(anchorObj.startTime)) + 'px';
          anchorObjButton.style.top = i * ((60 - ((numberOfLayer - 1) * 6)) / numberOfLayer + 6) + 'px';
          ...
          if (anchorObj.display == 'inTime') {
            anchorObjButton.onclick = () => { zoomToSelectedAnchor(anchorObj.lat, anchorObj.lon, anchorObj.floor)};
          };
  ...
  const zoomToSelectedAnchor = (lat: number, lon: number, floor: number) => {
    try {
      if (layerFloorplanVisible && floor != undefined) {setSelectedFloor(floor)};
  ...
  return (
    <IonPage>
      ...
      <IonContent fullscreen>
        ...
        <IonButton id="layerMenuButton" size="large" expand="block" fill="solid" onClick={switchLayerSelection}>
          <IonIcon icon={layersOutline} size="large"></IonIcon>
        </IonButton>
        {/* button background swisstopo */}
        <IonButton id="layerMenuButtonST" size="default" expand="block" fill="solid" onClick={selectBackgroundST}>
          <IonIcon icon={mapOutline} size="large"></IonIcon>
        </IonButton>
        {/* button background map floorplan */}
        <IonButton id="layerMenuButtonFP" size="default" expand="block" fill="solid" onClick={selectBackgroundFP}>
      ...
      <IonFooter id="footerTimeLineFilter" class="ion-padding">
        <IonDatetimeButton id="filterMenuDateSelection" datetime="datetime"></IonDatetimeButton>
        ...
        <div id="anchorDisplaySlider"></div>
          <div id="TimeSliderControl">
            ...
            <input id="TimeStampLeft" 
                   className="TimeSlider" 
                   type="range" 
                   value={startTimeFilter} 
                   min="0" 
                   max="24" 
                   onInput={(e) => {timeSliderLeftOnInput(e)}} 
                   // onBlur={() => timeSliderLeftOnBlur()} -> don't work with touch
                   onChange={(e) => {Number(e.target.value) < endTimeFilter? setStartTimeFilter(Number(e.target.value)): setStartTimeFilter(startTimeFilter)}}
            ...
            <input id="TimeStampRight" 
                   className="TimeSlider" 
                   type="range" 
                   value={endTimeFilter} 
                   min="0" 
                   max="24" 
                   onInput={(e) => {timeSliderRightOnInput(e)}} 
                   // onBlur={() => timeSliderRightOnBlur()} -> don't work with touch
                   onChange={(e) => {Number(e.target.value) > startTimeFilter? setEndTimeFilter(Number(e.target.value)): setEndTimeFilter(endTimeFilter)}}
        ...
        <IonModal keepContentsMounted={true} id="dialogSelectFilterDate">
         <IonHeader>
            <IonToolbar>
              ...
              <IonButtons slot="end">
                <IonButton onClick={() => (document.getElementById("dialogSelectFilterDate") as HTMLIonModalElement).dismiss()}>
                  <IonIcon icon={closeOutline} size="large"></IonIcon>
                </IonButton>
              </IonButtons>
            </IonToolbar>
          </IonHeader>
          <IonDatetime id="datetime" presentation="date" onIonChange={(e) => setSelectedDayFilter(new Date(e.target.value + '.000Z'))}></IonDatetime>
          <IonFooter class="ion-padding">
            <IonButton onClick={() => {(document.getElementById('dialogSelectFilterDate')! as HTMLIonModalElement).dismiss()}} expand="full" color="primary">Speichern</IonButton>
          </IonFooter>
        </IonModal>
